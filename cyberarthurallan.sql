...
CREATE DEFINER=`CyberArthurAllan`@`%` PROCEDURE `proc_getQuestions`(IN `Quiz` VARCHAR(50))
    NO SQL
BEGIN
...
CREATE DEFINER=`CyberArthurAllan`@`%` PROCEDURE `proc_getQuiz`(IN `Difficulte` VARCHAR(11), IN `Theme` VARCHAR(50))
    NO SQL
BEGIN
...
CREATE DEFINER=`CyberArthurAllan`@`%` PROCEDURE `proc_getReponses`(IN `Question` VARCHAR(200))
    NO SQL
BEGIN
...
CREATE DEFINER=`CyberArthurAllan`@`%` PROCEDURE `proc_getTop`(IN `numeroQuiz` INT)
BEGIN
...
CREATE DEFINER=`CyberArthurAllan`@`%` FUNCTION `func_addPropositions`(`libelle` VARCHAR(200)) RETURNS int(11)
    NO SQL
BEGIN
    DECLARE retour INT;
...
CREATE DEFINER=`CyberArthurAllan`@`%` FUNCTION `func_addQuestion`(`question` VARCHAR(200), `lvlDifficulte` VARCHAR(1), `letheme` INT(20)) RETURNS int(11)
    NO SQL
BEGIN
DECLARE retour INT;
...
CREATE DEFINER=`CyberArthurAllan`@`%` FUNCTION `func_addQuestionQuiz`(`numeroQuestion` INT, `numeroQuiz` INT) RETURNS varchar(30) CHARSET utf8
    NO SQL
BEGIN
...
CREATE DEFINER=`CyberArthurAllan`@`%` FUNCTION `func_addReponse`(`numeroQuestion` INT(11), `numeroProposition` INT(11), `bonneReponse` TINYINT(1)) RETURNS varchar(30) CHARSET utf8
    NO SQL
BEGIN
...
CREATE DEFINER=`CyberArthurAllan`@`%` FUNCTION `func_addResult`(`numeroQuiz` INT, `idJoueur` INT, `nbBonnesRep` INT) RETURNS varchar(30) CHARSET utf8
    NO SQL
BEGIN
DECLARE retour varchar(30) DEFAULT "valide";
...
CREATE DEFINER=`CyberArthurAllan`@`%` FUNCTION `func_addSection`(`nom` VARCHAR(20), `annee` VARCHAR(20), `specialite` VARCHAR(50)) RETURNS varchar(30) CHARSET utf8
    NO SQL
BEGIN
...
CREATE DEFINER=`CyberArthurAllan`@`%` FUNCTION `func_isAdmin`(`idJoueur` INT) RETURNS tinyint(1)
    NO SQL
BEGIN
